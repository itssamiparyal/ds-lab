...
void unionFuzzySet(char set1[], float value1[], int n1,
                   char set2[], float value2[], int n2)
{
    char resultSet[100];
    float resultValue[100];
    int k = 0;

    // Copy set A into result
    for (int i = 0; i < n1; i++)
    {
        resultSet[k] = set1[i];
    ...
    for (int i = 0; i < n2; i++)
    {
        ...
        for (int j = 0; j < n1; j++)
        {
            if (set2[i] == set1[j])
            {
                ...
                // Union uses maximum membership value
        ...
        // If element is not present in A, add it
        if (!found)
        {
            resultSet[k] = set2[i];
...
void intersectionFuzzySet(char set1[], float value1[], int n1,
                          char set2[], float value2[], int n2)
{
    char resultSet[100];
    float resultValue[100];
    ...
    for (int i = 0; i < n1; i++)
    {
        for (int j = 0; j < n2; j++)
        {
            if (set1[i] == set2[j])
            {
                ...
                // Intersection uses minimum membership value
                resultValue[k] =
                    value1[i] < value2[j] ? value1[i] : value2[j];
...
int main()
{
    ...
    printf("Enter elements and membership values for A:\n");
    ...
    printf("Enter elements and membership values for B:\n");
